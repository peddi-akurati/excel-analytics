...
All outputs remain available in DataFrames and are also appended to Delta audit tables.
...
RECONCILIATION_CATALOG = "your_recon_catalog"
RECONCILIATION_SCHEMA = "your_recon_schema"

# Optional table-name prefix for audit tables.
RECONCILIATION_TABLE_PREFIX = "recon"
...
# One audit identity is generated for the entire execution.
...
# ============================================================
# AUDIT / RECONCILIATION TABLE HELPERS
# ============================================================

def audit_name_part(value):
    """
    Convert database/schema/table text into a safe Delta table-name component.
    """
...
def variable_schema_audit_suffix(base_suffix):
    """
    Raw sample/duplicate outputs contain business-table columns and therefore
    do not have a stable schema across unrelated tables.

    Use a source/target-table-specific audit table name so a field such as
    row_wid can safely be BIGINT in one business table and STRING in another
    without Delta schema merge conflicts.
    """
    return (
        f"{base_suffix}_"
        f"{audit_name_part(POSTGRES_TABLE)}_to_"
        f"{audit_name_part(DATABRICKS_TABLE)}"
    )


def reconciliation_table_name(suffix):
    """
    Build a fully-qualified reconciliation audit table name.
    """
    prefix = RECONCILIATION_TABLE_PREFIX.strip()
    ...
    return (
        f"`{RECONCILIATION_CATALOG}`."
        f"`{RECONCILIATION_SCHEMA}`."
        f"`{table_name}`"
    )
...
def ensure_reconciliation_schema():
    """
    Ensure the configured reconciliation schema exists.
    """
    spark.sql(
...
def add_audit_columns_spark(
    ...
    """
    Add standard audit columns to a Spark DataFrame.
    """
    return (
        dataframe
        .withColumn(
...
def add_audit_columns_pandas(
    ...
    """
    Add standard audit columns to a pandas DataFrame.
    """
    result = dataframe.copy()

    result["run_id"] = RUN_ID
    result["run_datetime"] = RUN_DATETIME.isoformat()
    result["check_name"] = check_name
    result["output_type"] = output_type
    result["source_database"] = POSTGRES_CONFIG["database"]
    result["source_schema"] = POSTGRES_SCHEMA
    result["source_table"] = (
        POSTGRES_TABLE
        if include_table_context
        else None
    )
    result["target_catalog"] = DATABRICKS_CATALOG
    result["target_schema"] = DATABRICKS_SCHEMA
    result["target_table"] = (
        DATABRICKS_TABLE
        if include_table_context
        else None
...
def persist_spark_output(
    ...
    """
    Append a Spark DataFrame to a Delta audit table.
    """
    if dataframe is None:
        return
    ...
    audited_df = add_audit_columns_spark(
        dataframe,
        check_name,
        output_type,
    ...
    table_name = reconciliation_table_name(
        table_suffix
    )
    ...
    try:
        ...
        print("\n" + "=" * 80)
        print("AUDIT TABLE WRITE FAILED")
        print("=" * 80)
        print(f"Audit table : {table_name}")
        ...
        print(
            "\nThis generally means the same audit table previously "
            "stored a business column with a different datatype."
        )
        raise audit_error

    print(
        f"Audit table appended: {table_name}"
    )


def persist_pandas_output(
    dataframe,
    table_suffix,
    check_name,
    output_type,
    ...
    """
    Append a pandas DataFrame to a Delta audit table.
    """
    if dataframe is None:
        return
    ...
    if dataframe.empty:
        print(
            f"Skipping empty audit output: "
            f"{table_suffix}"
        )
        return

    audited_pdf = add_audit_columns_pandas(
        dataframe,
        check_name,
        output_type,
        include_table_context=include_table_context
    ...
    audited_sdf = pandas_to_spark_safe(
        audited_pdf
    )
...
def cleanup_reconciliation_tables():
    """
    Truncate reconciliation audit tables at the start of the run
    when RECON_CLEANUP_FLAG is True.
    """
    if not RECON_CLEANUP_FLAG:
        return
    ...
    table_names = [
        ...
        if (
            ...
            and (
                not prefix
                or row["tableName"].startswith(prefix + "_")
            )
        )
    ]

    print("\\nRECON_CLEANUP_FLAG is enabled.")

    if not table_names:
        print("No reconciliation tables found to truncate.")
        return
    ...
    for table_name in table_names:
        ...
        spark.sql(
            f"TRUNCATE TABLE {full_name}"
        )

        print(
            f"Truncated reconciliation table: {full_name}"
        )
...
def run_reconciliation():
    ...
    print(
        f"Run Time   : {RUN_DATETIME.isoformat()}"
    )
    ...
    try:
        ...
        if check_flag in {
            ...
            persist_pandas_output(
                column_mismatch_summary_df,
                "sample_column_mismatch_summary",
                "Sample_Check",
                "column_mismatch_summary"
            )
