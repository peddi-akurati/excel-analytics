...
# Second Databricks schema used by Type_Check.
# Type_Check compares PostgreSQL against BOTH Databricks schemas.
...
def get_databricks_table_names(
    ...
    """
    Return all non-temporary tables from the requested Databricks schema.

    If schema_name is not supplied, DATABRICKS_SCHEMA is used.
    """
    schema_name = (
        schema_name
    ...
    tables_df = spark.sql(
        f"""
        SHOW TABLES IN
        `{DATABRICKS_CATALOG}`.`{schema_name}`
        """
    )
...
def databricks_type_check_for_table(
    ...
    """
    Get Databricks column metadata for one requested table/schema.
    """
    full_name = (
        f"{DATABRICKS_CATALOG}."
        f"{schema_name}."
...
def run_reconciliation():
    ...
    try:
        ...
        if check_flag in {
            ...
            print(
                f"Databricks schema 1    : "
                f"{DATABRICKS_SCHEMA}"
            )

            print(
                f"Databricks schema 2    : "
