...
def build_column_level_mismatches(
    ...
    Also prints diagnostics if a duplicate column label would
    otherwise cause pandas:
    ...
    # Primary-key columns are often also present in hash columns.
    # Never select the same column twice.
    ...
    duplicate_requested_columns = [
        ...
        if compare_columns.count(column) > 1
    ]
    ...
    mismatch_keys_pdf = spark_dataframe_to_pandas_safe(
        sample_reconciliation_df.filter(
            F.col("RECON_STATUS") == F.lit("HASH_MISMATCH")
        ),
        key_columns
    )
    ...
    # Defensive check for duplicate labels already present in source pandas DF.
    source_duplicate_labels = (
        postgres_sample_df.columns[
            postgres_sample_df.columns.duplicated()
        ]
        .tolist()
    )

    if source_duplicate_labels:
        print("\n" + "=" * 80)
        print("SAMPLE CHECK - DUPLICATE COLUMN LABEL DIAGNOSTIC")
        print("=" * 80)
        print(
            "Duplicate column labels found in PostgreSQL sample:",
            source_duplicate_labels
        )
        print("\nSample rows containing the duplicate-label issue:")
        display(
            spark.createDataFrame(
                postgres_sample_df.head(10)
            )
        )

        raise ValueError(
            "PostgreSQL sample contains duplicate column labels: "
            + ", ".join(source_duplicate_labels)
        )

    # This is the common failure pattern: key columns get selected once
    # as keys and again because they also participate in the row hash.
    originally_requested = (
        key_columns + common_hash_columns
    )

    repeated_selection_labels = sorted(
        {
            column
            for column in originally_requested
            if originally_requested.count(column) > 1
        }
    )

    if repeated_selection_labels:
        print("\n" + "=" * 80)
        print("SAMPLE CHECK - DUPLICATE SELECTION DIAGNOSTIC")
        print("=" * 80)
        print(
            "The following primary-key columns were also present "
            "in the hash-column list and would have been selected twice:"
        )
        print(repeated_selection_labels)

        print(
            "\nSample of HASH_MISMATCH keys affected by this selection:"
        )
        display(
            spark.createDataFrame(
                mismatch_keys_pdf.head(10)
            )
        )

        print(
            "\nThe script will automatically de-duplicate the selection "
            "and continue with column-level mismatch analysis."
        )
    ...
    # Defensive Databricks-to-pandas duplicate label check.
    dbx_duplicate_labels = (
        dbx_compare.columns[
            dbx_compare.columns.duplicated()
        ]
        .tolist()
    )

    if dbx_duplicate_labels:
        print("\n" + "=" * 80)
        print("SAMPLE CHECK - DATABRICKS DUPLICATE COLUMN LABEL DIAGNOSTIC")
        print("=" * 80)
        print(
            "Duplicate column labels found after Databricks selection:",
            dbx_duplicate_labels
        )
        print("\nSample rows:")
        display(
        ...
        raise ValueError(
            "Databricks comparison sample contains duplicate column labels: "
            + ", ".join(dbx_duplicate_labels)
        )
    ...
    # Key columns are used for matching, so compare only non-key hash columns.
