...
# Second Databricks target used by Type_Check.
# Type_Check compares PostgreSQL against BOTH Databricks targets.
DATABRICKS_CATALOG_2 = "your_second_catalog"
DATABRICKS_SCHEMA_2 = "your_second_schema"
...
# Type_Check table scope.
# Only these tables will be checked by Type_Check / All_Check.
# Can be a comma-separated string or a Python list.
#
# Examples:
# TYPE_TABLE_LIST = "claims,policies,members"
# TYPE_TABLE_LIST = ["claims", "policies", "members"]
TYPE_TABLE_LIST = "your_table"

# Type_Check ordinal-position behavior.
# False -> compare ordinal position and datatype.
# True  -> ignore ordinal position completely; ordinal mismatches
#          will not be generated and will not affect summary status.
IGNORE_ORDINAL_CHECK = False
...
def get_databricks_table_names(
    ...
    """
    Return all non-temporary tables from the requested Databricks
    catalog/schema.
    ...
    catalog_name = (
        ...
        else DATABRICKS_CATALOG
    )

    schema_name = (
        schema_name
        if schema_name is not None
        else DATABRICKS_SCHEMA
    )
...
def normalize_type_name(value):
    """
    Normalize PostgreSQL and Databricks datatype names so equivalent
    physical types compare consistently across systems.
    """
    if value is None:
        return None
...
# ============================================================
# TYPE CHECK - POSTGRES + TWO DATABRICKS SCHEMAS
# ============================================================

def get_requested_type_tables_three_way(
    conn
):
    """
    Resolve TYPE_TABLE_LIST against:
        1. PostgreSQL schema
        2. Databricks schema 1
        3. Databricks schema 2

    Matching is case-insensitive.

    Returns a list of dictionaries, one per requested table,
    containing the resolved physical table name in each system.
    """
    requested_tables = parse_column_list(
        TYPE_TABLE_LIST
    )
    ...
    if not requested_tables:
        raise ValueError(
            "TYPE_TABLE_LIST must contain at least one table "
            "for Type_Check."
        )
    ...
    dbx1_tables = get_databricks_table_names(
        schema_name=DATABRICKS_SCHEMA,
        catalog_name=DATABRICKS_CATALOG
    )

    dbx2_tables = get_databricks_table_names(
        schema_name=DATABRICKS_SCHEMA_2,
        catalog_name=DATABRICKS_CATALOG_2
    )
    ...
    dbx1_map = {
        table.lower(): table
        for table in dbx1_tables
    }

    dbx2_map = {
        table.lower(): table
        for table in dbx2_tables
    }
        ...
        resolved.append(
            {
                ...
                "databricks_schema_1_table":
                    dbx1_map.get(
                        key
                    ),

                "databricks_schema_2_table":
                    dbx2_map.get(
                        key
                    )
            }
...
def postgres_type_check_for_table(
    conn,
    table_name
):
    """
    Get PostgreSQL column metadata for one requested table.
    """
    query = """
        SELECT
            table_schema,
...
def databricks_type_check_for_table(
    catalog_name,
    schema_name,
    table_name
):
    """
    Get Databricks column metadata for one requested catalog/schema/table.
    """
    full_name = (
        f"{catalog_name}."
        f"{schema_name}."
...
def _prepare_pg_type_pdf(
    pg_df
):
    """
    Prepare PostgreSQL type metadata for three-way merge.
    """
    result = pg_df.copy()
...
def _prepare_dbx_type_pdf(
    dbx_sdf,
    suffix
):
    """
    Prepare one Databricks schema's metadata for three-way merge.
    """
    result = dbx_sdf.toPandas()
    ...
    result[
        f"databricks_{suffix}_normalized_type"
    ] = result[
        "type_name"
    ].apply(
        normalize_type_name
    ...
    rename_map = {
        ...
        "schema":
            f"schema_{suffix}",
        "table_name":
            f"table_name_{suffix}",
        "ordinal_position":
            f"ordinal_position_{suffix}",
        "column_name":
            f"column_name_{suffix}",
        "data_type":
            f"data_type_{suffix}",
        "type_name":
            f"type_name_{suffix}",
...
def classify_pg_vs_dbx(
    row,
    dbx_suffix
):
    """
    Compare one PostgreSQL column against one Databricks target.

    When IGNORE_ORDINAL_CHECK = True:
        - ordinal position is ignored,
        - ORDINAL_MISMATCH is never returned,
        - ORDINAL_AND_TYPE_MISMATCH is reduced to TYPE_MISMATCH.
    """
    pg_col = row.get(
        "column_name"
    )
    ...
    dbx_type = row.get(
        f"databricks_{dbx_suffix}_normalized_type"
    )
    ...
    # Ordinal position is intentionally ignored.
    if IGNORE_ORDINAL_CHECK:
        return (
            "MATCH"
            if type_match
            else "TYPE_MISMATCH"
    ...
    dbx_pos = row.get(
        f"ordinal_position_{dbx_suffix}"
    )
...
def reconcile_type_three_way_for_table(
    requested_table,
    postgres_type_df,
    dbx1_type_df,
    dbx2_type_df
    ...
    """
    Three-way Type_Check for one requested table.

    PostgreSQL is the baseline.
    Each Databricks schema is independently compared to PostgreSQL.
    """
    pg = _prepare_pg_type_pdf(
        postgres_type_df
    )

    dbx1 = _prepare_dbx_type_pdf(
        dbx1_type_df,
        "schema_1"
    )

    dbx2 = _prepare_dbx_type_pdf(
        dbx2_type_df,
        "schema_2"
    )
    ...
    def three_way_status(row):
        ...
        if (
            ...
            return "MISMATCH_SCHEMA_1_ONLY"

        if (
            s1 == "MATCH"
            and s2 != "MATCH"
        ):
            return "MISMATCH_SCHEMA_2_ONLY"

        return "MISMATCH_BOTH_SCHEMAS"
    ...
    return pd.DataFrame(
        {
            ...
            "databricks_schema_1_column":
                merged[
                    "column_name_schema_1"
                ],

            "databricks_schema_1_ordinal_position":
                merged[
                    "ordinal_position_schema_1"
                ],

            "databricks_schema_1_data_type":
                merged[
                    "data_type_schema_1"
                ],

            "databricks_schema_1_type_name":
                merged[
                    "type_name_schema_1"
                ],

            "databricks_schema_1_normalized_type":
                merged[
                    "databricks_schema_1_normalized_type"
                ],
            ...
            "databricks_schema_2_column":
                merged[
                    "column_name_schema_2"
                ],

            "databricks_schema_2_ordinal_position":
                merged[
                    "ordinal_position_schema_2"
                ],

            "databricks_schema_2_data_type":
                merged[
                    "data_type_schema_2"
                ],

            "databricks_schema_2_type_name":
                merged[
                    "type_name_schema_2"
                ],

            "databricks_schema_2_normalized_type":
                merged[
                    "databricks_schema_2_normalized_type"
                ],
...
def empty_dbx_type_df():
    """
    Return an empty Databricks type DataFrame with the expected schema.
    """
    schema = T.StructType([
        T.StructField(
            "catalog",
...
def type_check_defined_tables_three_way(
    conn
):
    """
    Run Type_Check only for TYPE_TABLE_LIST across:
        PostgreSQL
        Databricks schema 1
        Databricks schema 2
    """
    resolved_tables = (
        get_requested_type_tables_three_way(
            conn
        )
    )
    ...
    for index, table_info in enumerate(
        resolved_tables,
        start=1
    ):
        requested_table = table_info[
            "requested_table"
        ]
        ...
        dbx1_table = table_info[
            "databricks_schema_1_table"
        ]

        dbx2_table = table_info[
            "databricks_schema_2_table"
        ]

        print(
            f"Type_Check {index}/"
            f"{len(resolved_tables)}: "
            f"{requested_table}"
        )
        ...
        # Databricks schema 1 metadata
        if dbx1_table is not None:
            dbx1_df = (
                databricks_type_check_for_table(
                    DATABRICKS_CATALOG,
                    DATABRICKS_SCHEMA,
                    dbx1_table
                )
            ...
            dbx1_df = empty_dbx_type_df()

        # Databricks schema 2 metadata
        if dbx2_table is not None:
            dbx2_df = (
                databricks_type_check_for_table(
                    DATABRICKS_CATALOG_2,
                    DATABRICKS_SCHEMA_2,
        ...
        # If PostgreSQL itself is missing, build a synthetic one-row
        # status record rather than attempting a column comparison.
        if pg_table is None:
        ...
        # If either Databricks table is absent, the empty metadata frame
        # naturally yields missing-column status. Add table-level override.
        recon_pdf = (
            reconcile_type_three_way_for_table(
                requested_table,
                pg_df,
                dbx1_df,
                dbx2_df
        ...
        if dbx1_table is None:
            recon_pdf[
                "STATUS_SCHEMA_1"
            ] = "TABLE_MISSING_IN_DATABRICKS_SCHEMA_1"

        if dbx2_table is None:
            recon_pdf[
                "STATUS_SCHEMA_2"
            ] = "TABLE_MISSING_IN_DATABRICKS_SCHEMA_2"

        def recompute_three_way(row):
            s1 = row[
                "STATUS_SCHEMA_1"
            ]
    ...
    if dbx1_outputs:
        dbx1_all_sdf = dbx1_outputs[0]

        for df in dbx1_outputs[1:]:
            dbx1_all_sdf = (
                dbx1_all_sdf
                .unionByName(
                    df,
    ...
    if dbx2_outputs:
        dbx2_all_sdf = dbx2_outputs[0]

        for df in dbx2_outputs[1:]:
            dbx2_all_sdf = (
                dbx2_all_sdf
                .unionByName(
                    df,
    ...
    for requested_table in parse_column_list(
        TYPE_TABLE_LIST
    ):
        subset = reconciliation_pdf[
            reconciliation_pdf[
                "requested_table"
        ...
        summary_rows.append(
            {
                ...
                "schema_1_mismatch_count":
                    schema_1_mismatch_count,

                "schema_1_status":
                    (
                        "MATCH"
                        if schema_1_mismatch_count == 0
                        and len(subset) > 0
                        else "MISMATCH"
                    ),
                ...
                "schema_2_mismatch_count":
                    schema_2_mismatch_count,

                "schema_2_status":
                    (
                        "MATCH"
                        if schema_2_mismatch_count == 0
                        and len(subset) > 0
                        else "MISMATCH"
                    ),
                ...
                "three_way_mismatch_count":
                    three_way_mismatch_count,

                "three_way_status":
                    (
                        "MATCH"
                        if three_way_mismatch_count == 0
                        and len(subset) > 0
                        else "MISMATCH"
                    )
            }
    ...
    summary_sdf = pandas_to_spark_safe(
        summary_pdf
    )
...
def run_reconciliation():
    ...
    try:
        ...
        if check_flag in {
            ...
            print(
                f"Databricks target 1    : "
                f"{DATABRICKS_CATALOG}."
                f"{DATABRICKS_SCHEMA}"
            )

            print(
                f"Databricks target 2    : "
                f"{DATABRICKS_CATALOG_2}."
                f"{DATABRICKS_SCHEMA_2}"
            )

            print(
                f"Ignore ordinal check   : "
                f"{IGNORE_ORDINAL_CHECK}"
            )
            ...
            (
                ...
                type_check_defined_tables_three_way(
                    conn
                )
            )
            ...
            if pg_types.empty:
                print(
                    "No PostgreSQL metadata available."
                )
            else:
                display(
                    pandas_to_spark_safe(
            ...
            print(
                "\nThree-Way Type Reconciliation Summary:"
            )
